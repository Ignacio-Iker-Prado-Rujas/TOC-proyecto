...
\section{Synthesis Report}
	...
	facilitan el código y su eliminación exigiría un tiempo que no merece la pena.
...
Para cuadrar en la pantalla lo que leemos de memoria hicimos varios cálculos. Tenemos dos referencias: una en $(0, 0)$, que es la pantalla entera y otra en $(5, 111)$, que es la zona de juego (sin los bordes amarillos). La zona de juego tiene vértices en $(5, 111), (260, 111), (5, 366), (260, 366)$. Por tanto $hcnt \in [5, 260]$ y $vcnt \in [111, 366]$.
...
\item \texttt{Nivel 2: }en este nivel se cargan el fondo de laboratorio y los obstáculos de tuberías. Al conseguir  $10+15n<60,\ n\in\mathbb{N}$ monedas se pasará al \texttt{nivel 3}, activando a su vez \textit{bloquea\_obstáculos}.
\item \texttt{Nivel 3: }en este nivel se cargan el fondo con nubes y montañas y los obstáculos de bolas de fuego. Al conseguir $15n<60,\ n\in\mathbb{N}$ monedas se pasará al \texttt{nivel 1}, y una vez se haya llegado a las 60 no se cambiará de nivel, aunque el juego continuará. 
\end{itemize}
...
Hemos conseguido reproducir fidedignamente el videojuego \emph{Jetpack Joyride}, incluyendo varios niveles diferentes, posibilidad de recoger monedas (y contar cuántas hemos recogido)...

Orgullosos de algo en particular...
Orgullosos de utilizar fondos reales creados por la mejor diseñadora gráfica del doble grado, así como de unas transiciones entre mapas que te dejan tiempo a acostumbrarte a la distribución de los nuevos obstáculos. Orgullosos de jugar con un muñeco real en vez de mover un pixel. Orgullosos de unos choques tan
...
\begin{itemize}
	\item Menú principal.
	\item Posibilidad de elegir personaje. Se barajaron varias opciones, incluso llegando a dibujarlas (cortesía de Daniel Báscones) como la de la imagen, pero no fue posible incluirlo.
	
	\begin{figure}[!h]
		\centering
		\includegraphics[scale=3]{natalia.png}
		\caption{Otro personaje del juego: Natbarry.}
	\end{figure}
	...
	\item Mejora de la exactitud de los choques. Nótese que la forma en que están implementados los choques se aproxima la figura de Barry como un rectángulo. Esto podría mejorarse con una aproximación con más segmentos a su figura, pero llevaría muchos más cálculos e implementación que no añaden demasiado. Ésta inexactitud se nota sobre todo al chocar con la parte de atrás de los pies del muñeco.
\end{itemize}
...
Ha sido de gran utilidad el software de la Universidad Rey Juan Carlos que permite pasar una imagen en formato PPM (Portable Pixel Map) a ROM. Esto ha posibilitado la carga de mapas muy realistas y que aprovechan al máximo la capacidad de la FPGA.
